...
GRANT SELECT, INSERT, DELETE, UPDATE ON CPSC_431_HW3.* TO 'nguyentran'@'localhost' IDENTIFIED BY 'nguyen';
...
(
  ...
  CONSTRAINT FORPLAYER FOREIGN KEY (Player) REFERENCES TeamRoster(ID),
  CONSTRAINT CHECKPLAYTIMEMIN CHECK (PlayingTimeMin <= 40),
  CONSTRAINT CHECKPLAYTIMESEC CHECK (PlayingTimeSec < 60)
);
